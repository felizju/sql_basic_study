...
CREATE TABLE users (
    ...
    user_rating_score NUMBER(1) NOT NULL, -- 평가점수
    user_role VARCHAR2(10) NOT NULL, -- 권한(ADMIN/USER)
    user_created_date DATE DEFAULT SYSDATE, -- 생성일자
    user_session_id VARCHAR2(30), -- 세션아이디
    user_limit_time DATE -- 자동로그인기간
);
...
-- 게시판 테이블 board
CREATE SEQUENCE board_seq-- 게시판 seq

CREATE TABLE board (
    board_no NUMBER(5) PRIMARY KEY, -- 게시판 번호
    user_id VARCHAR2(20) NOT NULL, -- 아이디
    user_nickname VARCHAR2(20) NOT NULL, -- 닉네임
    board_title VARCHAR2(100) NOT NULL, -- 게시판 제목
    board_content VARCHAR2(2000) NOT NULL, -- 게시판 내용
    board_date DATE DEFAULT SYSDATE, -- 게시판 작성일
    CONSTRAINT fk_board_user_id FOREIGN KEY(user_id) REFERENCES users(user_id),
    CONSTRAINT fk_board_user_nickname FOREIGN KEY(user_nickname) REFERENCES users(user_nickname)
);


-- 댓글 테이블 reply
CREATE SEQUENCE reply_seq; -- 댓글 seq

CREATE TABLE reply(
    reply_no NUMBER(5) PRIMARY KEY, -- 댓글 번호
    replyt_content VARCHAR2(2000) NOT NULL, -- 댓글 내용
    reply_date DATE DEFAULT SYSDATE, -- 댓글 작성일
    board_no NUMBER(5)  NOT NULL,, -- 체크리스트번호
    user_id VARCHAR2(20)  NOT NULL,, -- 아이디
    user_nickname VARCHAR2(20)  NOT NULL,, -- 닉네임
    CONSTRAINT fk_reply_board_no FOREIGN KEY(board_no) REFERENCES board(board_no),
    CONSTRAINT fk_reply_user_id FOREIGN KEY(user_id) REFERENCES users(user_id),
    CONSTRAINT fk_reply_user_nickname FOREIGN KEY(user_nickname) REFERENCES users(user_nickname)
);


-- 게시판 첨부파일 테이블 board_file
CREATE SEQUENCE board_file_seq; -- 게시판 첨부파일 seq

CREATE TABLE board_file(
    board_file_no NUMBER(5) PRIMARY KEY, -- 게시판 파일 번호
    board_file_name VARCHAR2(1000) NOT NULL, -- 게시판 파일명
    board_file_date DATE DEFAULT SYSDATE, -- 파일 업로드 날짜
    board_file_size NUMBER(10) NOT NULL, -- 파일 크기
    board_no NUMBER(5)  NOT NULL,  -- 게시판 번호
    user_id VARCHAR2(20)  NOT NULL, -- 아이디
    user_nickname VARCHAR2(20)  NOT NULL,, -- 닉네임
    CONSTRAINT fk_board_file_board_no FOREIGN KEY(board_no) REFERENCES board(board_no),
    CONSTRAINT fk_board_file_user_id FOREIGN KEY(user_id) REFERENCES users(user_id),
    CONSTRAINT fk_board_file_user_nickname FOREIGN KEY(user_nickname) REFERENCES users(user_nickname)
);
...
CREATE TABLE check_list(
    ...
    check_list_type VARCHAR2(10) NOT NULL, -- 체크리스트 구분
